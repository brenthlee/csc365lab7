...
public class InnReservations {
    ...
    public void funcReq1() throws SQLException {
        String sql = "with DaysOccupiedByReservation as ( " + "select Code, " + "Room, " + "datediff( "
                + "least(CheckOut, current_date()), "
                + "greatest(CheckIn, date_add(current_date(), interval -180 day)) " + ") as days_occupied_in_last_180 "
                + "from lab7_reservations " + "), FixDaysOccupiedByReservation as ( "
                ...
                + "from DaysOccupiedByReservation " + "), DaysOccupiedByRoom as ( "
                + "select Room, sum(days_occupied_in_last_180) as occupied_sum " + "from FixDaysOccupiedByReservation "
                + "group by Room " + "), FixedDaysOccupiedByRoom as ( "
                + "select Room, (case when (occupied_sum > 0) then occupied_sum else 0 end) as occupied_sum "
                + "from DaysOccupiedByRoom " + "), PopularityScoreByRoom as ( "
                + "select Room, round((occupied_sum / 180), 2) as popularity_score " + "from FixedDaysOccupiedByRoom "
                + "), RoomNextAvail as ( "
                ...
                + "from lab7_reservations " + "group by Room " + "), MostRecentCheckout as ( "
                + "select Room, max(CheckOut) as most_recent_checkout " + "from lab7_reservations "
                + "where CheckOut <= current_date() " + "group by Room " + "), LengthOfStay as ( "
                + "select Room, datediff(CheckOut, CheckIn) as length_of_stay "
                + "from MostRecentCheckout natural join lab7_reservations " + "where most_recent_checkout = CheckOut "
                + ") "
                ...
                + "where Room = RoomCode " + "order by popularity_score desc;";
    ...
    public void funcReq2() throws SQLException, IllegalArgumentException {
        String sqlBase = "with OverlappingRooms as ( " + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <=  ?  and  ?  < CheckOut) " + "), OverlappingRooms_ShiftRight as ( "
                + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <= date_add( ? , interval 1 day) and date_add( ? , interval 1 day) < CheckOut) "
                + "), OverlappingRooms_ShiftLeft as ( " + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <= date_add( ? , interval -1 day) and date_add( ? , interval -1 day) < CheckOut) "
                + "), OverlappingRooms_ShortenEnd as ( " + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <= date_add( ? , interval -1 day) and ? < CheckOut) "
                + "), OverlappingRooms_ShortenStart as ( " + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <= ? and date_add( ? , interval 1 day) < CheckOut) "
                + "), OverlappingRooms_ShiftRightTwo as ( " + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <= date_add( ? , interval 2 day) and date_add( ? , interval 2 day) < CheckOut) "
                + "), OverlappingRooms_ShiftLeftTwo as ( " + "select distinct Room " + "from lab7_reservations "
                + "where (CheckIn <= date_add( ? , interval -2 day) and date_add( ? , interval -2 day) < CheckOut) "
                + "), AvailableRooms as ( " + "select RoomCode as Room, " + " ?  as CheckIn, " + " ?  as CheckOut, "
                + "0 as Priority " + "from lab7_rooms " + "where RoomCode not in ( "
                + "select Room from OverlappingRooms " + ") " + "), AvailableRooms_ShiftRight as ( "
                + "select RoomCode as Room, " + "date_add( ? , interval 1 day) as CheckIn, "
                + "date_add( ? , interval 1 day) as CheckOut, " + "2 as Priority " + "from lab7_rooms "
                + "where RoomCode not in ( " + "select Room from OverlappingRooms_ShiftRight " + ") "
                + "), AvailableRooms_ShiftLeft as ( " + "select RoomCode as Room, "
                + "date_add( ? , interval -1 day) as CheckIn, " + "date_add( ? , interval -1 day) as CheckOut, "
                + "2 as Priority " + "from lab7_rooms " + "where RoomCode not in ( "
                + "select Room from OverlappingRooms_ShiftLeft " + ") " + "), AvailableRooms_ShortenEnd as ( "
                + "select RoomCode as Room, " + " ?  as CheckIn, " + "date_add( ? , interval -1 day) as CheckOut, "
                + "1 as Priority " + "from lab7_rooms " + "where RoomCode not in ( "
                + "select Room from OverlappingRooms_ShortenEnd " + ") " + "), AvailableRooms_ShortenStart as ( "
                + "select RoomCode as Room, " + "date_add( ? , interval 1 day) as CheckIn, " + " ?  as CheckOut, "
                + "1 as Priority " + "from lab7_rooms " + "where RoomCode not in ( "
                + "select Room from OverlappingRooms_ShortenStart " + ") " + "), AvailableRooms_ShiftRightTwo as ( "
                + "select RoomCode as Room, " + "date_add( ? , interval 2 day) as CheckIn, "
                + "date_add( ? , interval 2 day) as CheckOut, " + "3 as Priority " + "from lab7_rooms "
                + "where RoomCode not in ( " + "select Room from OverlappingRooms_ShiftRightTwo " + ") "
                + "), AvailableRooms_ShiftLeftTwo as ( " + "select RoomCode as Room, "
                + "date_add( ? , interval -2 day) as CheckIn, " + "date_add( ? , interval -2 day) as CheckOut, "
                + "3 as Priority " + "from lab7_rooms " + "where RoomCode not in ( "
                + "select Room from OverlappingRooms_ShiftLeftTwo " + ") " + "), AllAvailByPriority as ( " + "select * "
                + "from AvailableRooms " + "union " + "select * " + "from AvailableRooms_ShiftRight " + "union "
                + "select * " + "from AvailableRooms_ShiftLeft " + "union " + "select * "
                + "from AvailableRooms_ShortenEnd " + "union " + "select * " + "from AvailableRooms_ShortenStart "
                + "union " + "select * " + "from AvailableRooms_ShiftRightTwo " + "union " + "select * "
                + "from AvailableRooms_ShiftLeftTwo " + "), AllAvailPlusInfo as ( " + "select *  "
                + "from AllAvailByPriority, lab7_rooms " + "where RoomCode = Room " + "order by Priority, Room "
                + "), CheckOccupancy as ( " + "select Room, CheckIn, CheckOut, Priority, bedType, basePrice "
                + "from AllAvailPlusInfo " + "where maxOcc >= ? " + "), FuzzyMatch as ( " + "select * "
                + "from CheckOccupancy " + "), FilterBed as ( " + "select * " + "from CheckOccupancy "
                + "where bedType like ? " + "), FilterRoom as ( " + "select * " + "from FilterBed "
                + "where Room like ? " + "), ExactMatch as ( " + "select * " + "from FilterRoom "
                + "where Priority = 0 " + ") ";

        String sqlExact = "select * " + "from ExactMatch;";

        String sqlFuzzy = "select * " + "from FuzzyMatch " + "limit 5;";
        ...
        try (Connection dbConnection = DriverManager.getConnection(url, name, pass)) {
            ...
            if (exactResult.next()) {
                ...
                int index = 1;
                System.out.format("\n%-12s | %-12s | %-12s | %12s | %12s\n", "Listing #", "Room Code", "Available From",
                        "Available To", "Bed Type");
                do {
                ...
                if (selection == 0) {
                    ...
                    PreparedStatement insertReservation = dbConnection.prepareStatement(
                            "insert into lab7_reservations (CODE, Room, CheckIn, Checkout, Rate, LastName, FirstName, Adults, Kids) value ( ?, ?, ?, ?, ?, ?, ?, ?, ? );");

                    insertReservation.setInt(1, reservationCode);
                    insertReservation.setString(2, exactResult.getString("Room"));

                    insertReservation.setDate(3, java.sql.Date.valueOf(exactResult.getString("CheckIn")),
                            java.util.Calendar.getInstance());
                    insertReservation.setDate(4, java.sql.Date.valueOf(exactResult.getString("CheckOut")),
                            java.util.Calendar.getInstance());
                    ...
                    System.out.format("\n%-12d | %-12s | %-12s | %12s | %12d | %12s | %12s | %12d | %12d\n",
                            reservationCode, exactResult.getString("Room"), exactResult.getString("CheckIn"),
                            exactResult.getString("CheckOut"), exactResult.getInt("basePrice"), ln, fn, na, nc);
            ...
            // if there is no exact result, get the fuzzy result
                ...
                if (selection == 0) {
                    ...
                    insertReservation.setDate(3, java.sql.Date.valueOf(fuzzyResult.getString("CheckIn")),
                            java.util.Calendar.getInstance());
                    insertReservation.setDate(4, java.sql.Date.valueOf(fuzzyResult.getString("CheckOut")),
                            java.util.Calendar.getInstance());
                    ...
                    System.out.format("\n%-12d | %-12s | %-12s | %12s | %12d | %12s | %12s | %12d | %12d\n",
                            reservationCode, fuzzyResult.getString("Room"), fuzzyResult.getString("CheckIn"),
                            fuzzyResult.getString("CheckOut"), fuzzyResult.getInt("basePrice"), ln, fn, na, nc);
                    String selection2 = scan.next();
    ...
    // update reservation
    public void funcReq3() throws SQLException, IllegalArgumentException {
        // Setup
        Scanner scanner = new Scanner(System.in);
        Connection dbConnection = connect();
        dbConnection.setAutoCommit(false);
        ...
        if (validResrvation) {
            try {
                ...
                System.out.println("Incorrectly formatted input. Returning to Menu");
                return;
            }
        ...
        if (validResrvation) {
            ...
            if (tempInput.toUpperCase().equals("YES") || tempInput.toUpperCase().equals("Y")) {
                ...
                try {
                    ...
                    System.out.println(String.valueOf(updateRowsCount) + " reservation(s) canceled: "
                            + String.valueOf(reservationCode));
        ...
        try (Connection dbConnection = DriverManager.getConnection(url, name, pass)) {
            ...
            try (PreparedStatement pstmt = dbConnection.prepareStatement(searchSql)) {
                ...
                while (resInfoResult.next()) {
                    ...
                    System.out.format(
                            "%s %s Reservation Code: %d Room Code: %s Check-in: %s Check-out: %s Rate: %d Adults: %d Kids: %d Room Name: %s %n",
                            fnameResult, lnameResult, reservationCodeResult, roomCodeResult, CheckIn, CheckOut,
                            rateResult, AdultsResult, kidsResult, RoomNameResult);
                }
    ...
    public void funcReq6() throws SQLException {
        ...
        try (Connection dbConnection = DriverManager.getConnection(url, name, pass)) {
            ...
            String RevenueSql = "with b as ( " + "with a as( "
                    ...
                    + "SUM(CASE WHEN month = 12 THEN TotalCharge END) December " + "from a "
            ...
            try (PreparedStatement pstmt = dbConnection.prepareStatement(RevenueSql)) {
                ...
                System.out.format(
                        "%-6s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %n",
                        roomCol, JanuaryCol, febCol, marchcol, aprilCol, mayCol, juneCol, julyCol, augcol, septcol,
                        octCol, novCol, decCol, roomtotcol);
                ...
                while (resInfoResult.next()) {
                    ...
                    System.out.format(
                            "%-6s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %-10s | %n",
                            room, jan, feb, mar, apr, may, jun, july, aug, sept, oct, nov, dec, tot);
                }
